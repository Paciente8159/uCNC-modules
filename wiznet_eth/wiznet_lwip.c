...
#ifndef WIZNET_TX_QUEUE_SIZE
#define WIZNET_TX_QUEUE_SIZE 128
#endif
...
typedef struct
{
    ...
    uint16_t tx_tail;
    uint16_t tx_count;
    uint8_t tx_queue[WIZNET_TX_QUEUE_SIZE];
} wiz_hw_slot_t;
...
static void reset_tx_queue(uint8_t socket_number)
{
...
{
    return (uint16_t)(WIZNET_TX_QUEUE_SIZE -
                      hw_slots[socket_number].tx_count);
}

/* Enqueue short writes atomically. A normal protocol line is therefore never
 * cut in half merely because the queue is almost full. Writes larger than the
 * whole queue may still be accepted partially, as allowed by socket_device_t. */
static size_t enqueue_tx(uint8_t socket_number, const uint8_t *data,
                         size_t length)
{
    wiz_hw_slot_t *slot = &hw_slots[socket_number];
    uint16_t free_space = tx_queue_free(socket_number);
    size_t accepted;
    size_t first;

    if (length <= WIZNET_TX_QUEUE_SIZE && length > free_space)
    {
        return 0U;
    }
    ...
    first = WIZNET_TX_QUEUE_SIZE - slot->tx_head;
    if (first > accepted)
    {
        first = accepted;
    }
    memcpy(&slot->tx_queue[slot->tx_head], data, first);
    if (first < accepted)
    {
        memcpy(slot->tx_queue, data + first, accepted - first);
    }

    slot->tx_head = (uint16_t)((slot->tx_head + accepted) %
                               WIZNET_TX_QUEUE_SIZE);
    slot->tx_count = (uint16_t)(slot->tx_count + accepted);
    return accepted;
}

/* Start at most one hardware SEND operation. Bytes remain in the software
 * queue until the W5500 accepts them into its TX ring. Further queued bytes
 * are sent after SEND_OK is observed by service_client(). */
static int pump_tx(uint8_t socket_number)
{
    wiz_hw_slot_t *slot = &hw_slots[socket_number];
    uint16_t contiguous;
    int result;

    if (slot->tx_pending || slot->tx_count == 0U)
    {
        return 0;
    }

    contiguous = (uint16_t)(WIZNET_TX_QUEUE_SIZE - slot->tx_tail);
    if (contiguous > slot->tx_count)
    {
        contiguous = slot->tx_count;
    }

    result = wiznet_hw_socket_send(socket_number,
                                   &slot->tx_queue[slot->tx_tail],
                                   contiguous);
    if (result > 0)
    {
        slot->tx_tail = (uint16_t)((slot->tx_tail + (uint16_t)result) %
                                   WIZNET_TX_QUEUE_SIZE);
        slot->tx_count = (uint16_t)(slot->tx_count - (uint16_t)result);
        slot->tx_pending = true;
        WIZDGB("WIZnet backend: socket %u started hardware TX of %u bytes; %u queued\n",
               (unsigned int)socket_number, (unsigned int)result,
               (unsigned int)slot->tx_count);
    }
    return result;
}
...
static bool open_listener_socket(uint8_t listener_index)
{
    ...
    reset_tx_queue((uint8_t)socket_number);
                            ...
                            size_t length, int flags)
{
    ...
    {
        WIZDGB("WIZnet backend: socket %d send rejected, status=0x%X\n",
               index, (unsigned int)status);
        return SOCKET_DEVICE_CLOSED;
    }

    accepted = enqueue_tx((uint8_t)index, (const uint8_t *)data, length);
    if (accepted == 0U)
    {
        hw_slots[index].tx_backpressured = true;
        WIZDGB("WIZnet backend: socket %d TX queue full; %u/%u bytes queued\n",
               index, (unsigned int)hw_slots[index].tx_count,
               (unsigned int)WIZNET_TX_QUEUE_SIZE);
        return SOCKET_DEVICE_WOULD_BLOCK;
    }

    pump_result = pump_tx((uint8_t)index);
    if (pump_result < 0)
    {
        WIZDGB("WIZnet backend: socket %d failed to start queued TX\n", index);
        return SOCKET_DEVICE_ERROR;
    }

    WIZDGB("WIZnet backend: socket %d accepted %u/%u bytes into TX queue; %u queued\n",
           index, (unsigned int)accepted, (unsigned int)length,
           (unsigned int)hw_slots[index].tx_count);
    return (int)accepted;
}
...
static void service_listener(uint8_t socket_number, uint8_t status)
{
    ...
    else if (status == WIZ_SNSR_SYNRECV)
    {
        /* Normal passive-open transition: LISTEN -> SYNRECV -> ESTABLISHED.
         * Do not close or reopen the hardware socket while the TCP handshake
         * is in progress. A later bounded service pass will accept it after
         * the WIZnet controller reports ESTABLISHED. */
        WIZDGB("WIZnet backend: listener socket %u TCP handshake in progress\n",
               (unsigned int)socket_number);
    }
    else if (status != WIZ_SNSR_LISTEN)
    {
        WIZDGB("WIZnet backend: listener socket %u left LISTEN, status=0x%X; reopening\n",
...
static void service_client(uint8_t socket_number, uint8_t status)
{
    ...
    {
        WIZDGB("WIZnet backend: socket %u timeout interrupt, IR=0x%X\n",
    ...
    {
        WIZDGB("WIZnet backend: socket %u entered closing status 0x%X\n",
               (unsigned int)socket_number, (unsigned int)status);
    ...
    {
        WIZDGB("WIZnet backend: socket %u remote close, status=0x%X IR=0x%X\n",
               (unsigned int)socket_number, (unsigned int)status,
...
static void wiz_backend_service(void)
{
    ...
    if (hw_slots[socket_number].role != WIZ_ROLE_FREE)
    {
        ...
        {
            WIZDGB("WIZnet backend: socket %u status 0x%X -> 0x%X, role=%u\n",
                   (unsigned int)socket_number,
